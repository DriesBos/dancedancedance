...
- **`agent-browser`** — a CLI that drives a real Chrome. Knows
  ...
  fiber, vitals. This repo installs it locally; invoke it with
  `pnpm exec agent-browser`. Before driving it, run
  `pnpm exec agent-browser skills get core` once for the
  version-matched usage guide — don't guess subcommands from memory.
...
- Upgrade Next.js: `pnpm next upgrade`.
  Docs: https://nextjs.org/docs/app/getting-started/upgrading
  (version-16 guide:
  https://nextjs.org/docs/app/guides/upgrading/version-16)
- Install or upgrade `agent-browser`: `pnpm add -D agent-browser@latest`.
  Preflight expects to invoke the project-local CLI with `pnpm exec`.
...
1. **Open `agent-browser` at the target URL, restoring saved
   ...
   ```bash
   SESSION="$(pnpm exec agent-browser session id --scope worktree --prefix next-dev-loop)"
   ...
   and restore context; `pnpm exec agent-browser close` saves the cookie
   state so the next `open` restores it.
...
- A blank read, empty snapshot, `about:blank`, or a "no browser
  ...
  run `pnpm exec agent-browser --session "$SESSION" --restore close`,
  then open again. Don't fall back to `curl`; it bypasses the browser
  you're testing.
...
`pnpm exec agent-browser --session "$SESSION" --restore close`. `close`
saves that session's cookies and storage so the next loop's `--restore`
open keeps the user logged in. Leave `next dev` up for the next loop.
